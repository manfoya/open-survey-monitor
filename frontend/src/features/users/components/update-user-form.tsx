...
import { AlertCircle, RefreshCw, Check, ChevronsUpDown, Lock } from "lucide-react";
...
function getRequiredChefRoleLabel(userRole: string): string {
  switch (userRole) {
...
export default function UpdateUserForm({ user, canChangeChef, onSuccess }: UpdateUserFormProps) {
  ...
  // 2. Binding du Server Action
  // On lie l'ID de l'utilisateur à la fonction d'update
  const updateUserWithId = updateUserAction.bind(null, user.id);
  const [state, formAction, isPending] = useActionState(updateUserWithId, { success: false });

  // 3. Chargement des utilisateurs (nécessaire pour la liste ET pour afficher le nom du chef actuel)
  ...
  const eligibleChefs = useMemo(() => {
    const currentWeight = ROLE_WEIGHTS[user.role] || 0;
    const requiredChefWeight = currentWeight + 1;
    
    return users.filter((u) => 
      ROLE_WEIGHTS[u.role] === requiredChefWeight && 
      u.id !== user.id // Exclusion de soi-même
    );
  }, [user.role, user.id, users]);
  ...
  const currentChefName = useMemo(() => {
    ...
    return users.find(u => u.id.toString() === selectedChef)?.username || "Inconnu (ID invalide)";
  }, [selectedChef, users]);
        ...
        <div className="space-y-2 flex flex-col">
          <div className="flex items-center justify-between mb-1">
            <Label className="flex items-center gap-2">
              ...
              {!canChangeChef && <Lock className="h-3 w-3 text-muted-foreground" />}
            </Label>
          ...
          {canChangeChef ? (
            ...
            <>
                        ...
                        {eligibleChefs.map((chef) => (
                          ...
                          >
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-[10px] text-muted-foreground uppercase">{chef.role}</span>
                            </div>
                          </CommandItem>
                        ))}
                      </CommandGroup>
              ...
              {eligibleChefs.length === 0 && !isLoadingUsers && (
                 <p className="text-[10px] text-amber-600">Aucun responsable éligible trouvé.</p>
              )}
            </>
          ) : (
            /* --- MODE LECTURE SEULE --- */
            <div className="relative">
                <Input 
                    disabled 
                    value={isLoadingUsers ? "Chargement..." : (currentChefName || "Aucun chef assigné")} 
                ...
                <p className="text-[10px] text-muted-foreground mt-1">
                    Le changement de hiérarchie n&apos;est pas autorisé pour ce profil.
                </p>
            </div>
          )}
